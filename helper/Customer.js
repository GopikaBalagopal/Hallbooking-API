...
export async function getroomdetails(Room_id,setdate,
  ...
  return await client
    .db("Hallbooking")
    .collection("Customerdetails")
    .findOne({ Room_id: new ObjectId(Room_id), date: setdate,
        StartTime: StartTime,
        EndTime: EndTime });
}
...
export async function changeroomstatus(Room_id) {
  return await client.db("Hallbooking").collection("Rooms").updateOne({_id: new ObjectId(Room_id)},{ $set: {Status: "Booked"} });
}
